...
get_latest_version() {
    ...
    if [[ -z "$version" ]]; then
        warn "Could not fetch latest version from GitHub. Using v0.4.4"
        version="v0.4.4"
    fi
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --help|-h)
                ...
                echo "  curl -fsSL https://raw.githubusercontent.com/FrozenTear/nirify/master/install.sh | bash -s -- --source --prefix=/usr --version=0.4.4"
                echo ""
                exit 0
                ;;
            *)
