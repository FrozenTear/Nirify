...
impl super::super::App {
    ...
    pub(in crate::app) fn update_debug(&mut self, msg: DebugMessage) -> Task<Message> {
        ...
        match msg {
            ...
            DebugMessage::SetEmulateZeroPresentationTime(v) => {
                debug.emulate_zero_presentation_time = v
            }
            DebugMessage::SetSkipCursorOnlyUpdatesDuringVrr(v) => {
            ...
            DebugMessage::SetKeepLaptopPanelOnWhenLidIsClosed(v) => {
                debug.keep_laptop_panel_on_when_lid_is_closed = v
            }
            DebugMessage::SetDisableMonitorNames(v) => debug.disable_monitor_names = v,
            DebugMessage::SetForceDisableConnectorsOnResume(v) => {
                debug.force_disable_connectors_on_resume = v
            }
            DebugMessage::SetStrictNewWindowFocusPolicy(v) => {
            ...
            DebugMessage::SetDeactivateUnfocusedWindows(v) => {
                debug.deactivate_unfocused_windows = v
            }
            DebugMessage::SetForcePipewireInvalidModifier(v) => {
