...
pub fn get_windows() -> IpcResult<Vec<WindowInfo>> {
    ...
    match serde_json::from_str::<NiriResponse<WindowsResponse>>(&response) {
...
pub fn get_focused_window() -> IpcResult<Option<WindowInfo>> {
    ...
    match serde_json::from_str::<NiriResponse<FocusedWindowResponse>>(&response) {
        Ok(NiriResponse::Ok { inner }) => Ok(inner.window),
        Ok(NiriResponse::Err { error }) => {
            warn!(
        ...
        Err(e) => {
            warn!("Failed to parse focused window response: {}", e);
            Err(IpcError::ParseError(format!("{}", e)))
        }
    }
}
