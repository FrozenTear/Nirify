...
pub fn view(settings: &DebugSettings) -> Element<'_, Message> {
    ...
    let mut content = column![
        ...
        card(column![
            ...
            column![
                ...
                container(text("Override DRM device for rendering (e.g., /dev/dri/renderD128)").size(12)).style(muted_text_container),
    ...
    content = content.push(
        container(
            column![
                ...
                container(text("DRM devices to ignore (useful for GPU passthrough)").size(12)).style(muted_text_container),
            ].spacing(4)
        ).padding(12)
    );
    ...
    if ignore_drm.is_empty() {
        content = content.push(
            container(container(text("None configured").size(14)).style(muted_text_container)).padding([0, 12])
        );
    } else {
