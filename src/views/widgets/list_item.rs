...
pub fn list_item<Message: Clone + 'static>(
    ...
    let label_container = container(text(label).size(14)).style(move |theme: &iced::Theme| {
        let txt = theme.palette().text;
        let color = if is_selected {
            txt
        } else {
    ...
    if let Some(badge_text) = badge {
        content = content.push(container(text(badge_text).size(11)).padding([2, 6]).style(
            |theme: &iced::Theme| {
                let primary = theme.palette().primary;
                container::Style {
                    background: Some(iced::Background::Color(iced::Color { a: 0.3, ..primary })),
