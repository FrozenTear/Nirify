...
// Generate helper functions for TrackpointSettingModel
crate::impl_setting_builders!(TrackpointSettingModel);
...
pub fn setup(ui: &MainWindow, settings: Arc<Mutex<Settings>>, save_manager: Rc<SaveManager>) {
    ...
    // Generic combo callback
    {
        ...
        ui.on_trackpoint_setting_combo_changed(move |id, index| {
            ...
            // UI updates happen after lock is released
            if let Some(trackpoint_clone) = ui_update {
                if let Some(ui) = ui_weak.upgrade() {
                    sync_all_models(&ui, &trackpoint_clone);
                }
            }
        });
    }
