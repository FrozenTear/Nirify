//! Startup commands UI callbacks
//!
//! Handles spawn-at-startup command configuration including add, remove, select, and editing.
...
fn build_command_list_model(commands: &[StartupCommand]) -> ModelRc<SharedString> {
    let mut display_list = Vec::with_capacity(commands.len());
    for cmd in commands {
        display_list.push(SharedString::from(cmd.display()));
    }
    ModelRc::new(VecModel::from(display_list))
}

/// Sync current command to UI
fn sync_current_command(ui: &MainWindow, cmd: &StartupCommand) {
    ui.set_current_startup_command(SharedString::from(cmd.display()));
}

/// Set up startup command callbacks
pub fn setup(ui: &MainWindow, settings: Arc<Mutex<Settings>>, save_manager: Rc<SaveManager>) {
    ...
    // Add command callback
    {
        ...
        ui.on_startup_add_command(move || {
            match settings.lock() {
                Ok(mut s) => {
                    ...
                    if let Some(ui) = ui_weak.upgrade() {
                        ui.set_startup_command_list(build_command_list_model(&s.startup.commands));
                        ui.set_selected_startup_index(new_idx);

                        if let Some(cmd) = s.startup.commands.get(new_idx as usize) {
                            sync_current_command(&ui, cmd);
                        }
                    }
    ...
    // Remove command callback
    {
        ...
        let ui_weak = ui.as_weak();
        let save_manager = Rc::clone(&save_manager);
        ui.on_startup_remove_command(move |index| {
            ...
            match settings.lock() {
                Ok(mut s) => {
                    if idx < s.startup.commands.len() {
                        ...
                        // Update UI
                        if let Some(ui) = ui_weak.upgrade() {
                            ui.set_startup_command_list(build_command_list_model(
                                &s.startup.commands,
                            ));
                            ui.set_selected_startup_index(new_sel);
    ...
    // Select command callback
    {
        ...
        let selected_idx = Rc::clone(&selected_idx);
        let ui_weak = ui.as_weak();
        ui.on_startup_select_command(move |index| {
    ...
    // Command changed callback
    {
        ...
        let ui_weak = ui.as_weak();
        let save_manager = Rc::clone(&save_manager);
        ui.on_startup_command_changed(move |value| {
    ...
    // Reorder command callback (drag-and-drop)
    {
        ...
        let ui_weak = ui.as_weak();
        let save_manager = Rc::clone(&save_manager);
        ui.on_startup_reorder_command(move |from, to| {
            ...
            match settings.lock() {
                Ok(mut s) => {
                    ...
                    if let Some(ui) = ui_weak.upgrade() {
                        ui.set_startup_command_list(build_command_list_model(&s.startup.commands));
                        ui.set_selected_startup_index(insert_idx as i32);
