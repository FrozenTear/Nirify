...
pub fn overview_page(state: AppState) -> impl IntoView {
    ...
    // Callbacks
    let on_zoom = { let state = state.clone(); Rc::new(move |val: f64| {
        state.update_settings(|s| s.overview.zoom = val);
        state.mark_dirty_and_save(SettingsCategory::Overview);
    })};

    let on_backdrop_color = { Rc::new(move |val: String| {
        if val.is_empty() {
    ...
    Stack::vertical((
        section(
            "View",
            Stack::vertical((
                slider_row_with_callback("Zoom level", Some("Overview zoom (0.1 = zoomed out, 2.0 = zoomed in)"), zoom, 0.1, 2.0, 0.1, "x", Some(on_zoom)),
            )),
        ),
        section(
            "Appearance",
            Stack::vertical((
                color_row_with_callback("Backdrop color", Some("Background color behind workspace previews"), backdrop_color, Some(on_backdrop_color)),
            )),
        ),
    ))
