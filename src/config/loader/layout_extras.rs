...
/// Read an integer-valued field, falling back to a rounded float.
///
/// niri treats these numeric layout fields as FloatOrInt, so a hand-written
/// config using e.g. `softness 30.5` must not be silently dropped.
fn get_int_or_float(children: &KdlDocument, keys: &[&str]) -> Option<i64> {
    get_i64(children, keys).or_else(|| get_f64(children, keys).map(|f| f.round() as i64))
}
...
pub fn parse_layout_extras_from_children(l_children: &KdlDocument, settings: &mut Settings) {
    // Shadow - styling parsed unconditionally so it round-trips even when off.
    if let Some(shadow) = l_children.get("shadow") {
        if let Some(s_children) = shadow.children() {
            settings.layout_extras.shadow.enabled = !has_flag(s_children, &["off"]);

            if let Some(v) = get_int_or_float(s_children, &["softness"]) {
                settings.layout_extras.shadow.softness = v as i32;
            }
            if let Some(v) = get_int_or_float(s_children, &["spread"]) {
                settings.layout_extras.shadow.spread = v as i32;
            }

            // Offset (accept float fallback - niri offsets are FloatOrInt)
                ...
                for entry in offset.entries() {
                    if let Some(name) = entry.name() {
                        let val = entry
                            ...
                            .as_integer()
                            .map(|i| i as i32)
                            .or_else(|| entry.value().as_float().map(|f| f.round() as i32));
                        if let Some(val) = val {
                            match name.value() {
                                "x" => settings.layout_extras.shadow.offset_x = val,
                                "y" => settings.layout_extras.shadow.offset_y = val,
                                _ => {}
                            }
                        }
                    }
            ...
            // Only opt into the custom inactive color when the node is present.
            if s_children.get("inactive-color").is_some() {
                settings.layout_extras.shadow.use_inactive_color = true;
                load_color(
                    s_children,
                    &["inactive-color"],
            ...
            // draw-behind-window is a bool argument (Option<bool>): `false` disables.
            settings.layout_extras.shadow.draw_behind_window =
                has_flag(s_children, &["draw-behind-window"]);
        }
    }

    // Tab indicator - styling parsed unconditionally so it round-trips even when off.
    if let Some(ti) = l_children.get("tab-indicator") {
        if let Some(ti_children) = ti.children() {
            settings.layout_extras.tab_indicator.enabled = !has_flag(ti_children, &["off"]);
            ...
            if let Some(v) = get_int_or_float(ti_children, &["width"]) {
                settings.layout_extras.tab_indicator.width = v as i32;
            }
            if let Some(v) = get_int_or_float(ti_children, &["gap"]) {
                settings.layout_extras.tab_indicator.gap = v as i32;
            }
            if let Some(v) = get_int_or_float(ti_children, &["gaps-between-tabs"]) {
                settings.layout_extras.tab_indicator.gaps_between_tabs = v as i32;
            }
            if let Some(v) = get_int_or_float(ti_children, &["corner-radius"]) {
                settings.layout_extras.tab_indicator.corner_radius = v as i32;
            }
            // length total-proportion=<f64> (property on a child node)
            if let Some(length_node) = ti_children.get("length") {
                for entry in length_node.entries() {
                    if let Some(name) = entry.name() {
                        if name.value() == "total-proportion" {
                            if let Some(v) = entry
                                .value()
                                .as_float()
                                .or_else(|| entry.value().as_integer().map(|i| i as f64))
                            {
                                settings.layout_extras.tab_indicator.length_proportion = v as f32;
                            }
                        }
                    }
                }
            }
            // Colors are optional; presence marks the user opt-in (use_*_color).
            if let Some(cog) = load_color_or_gradient(ti_children, "active") {
                settings.layout_extras.tab_indicator.active = cog;
                settings.layout_extras.tab_indicator.use_active_color = true;
            }
            if let Some(cog) = load_color_or_gradient(ti_children, "inactive") {
                settings.layout_extras.tab_indicator.inactive = cog;
                settings.layout_extras.tab_indicator.use_inactive_color = true;
            }
            if let Some(cog) = load_color_or_gradient(ti_children, "urgent") {
                settings.layout_extras.tab_indicator.urgent = cog;
                settings.layout_extras.tab_indicator.use_urgent_color = true;
            }
    ...
    // Insert hint (supports gradient in niri) - styling parsed even when off.
